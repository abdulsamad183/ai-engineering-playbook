---
title: "ELBO & KL Regularization"
description: "The Evidence Lower Bound (ELBO) = reconstruction term − KL(q(z|x)||p(z)); KL prevents posterior collapse or over-regularization depending on weighting."
domain: generative-ai
tags: [vaes, generative-ai]
status: published
created: 2026-08-11
updated: 2026-08-12
version: "2.0"
...
# ELBO & KL Regularization

> The Evidence Lower Bound (ELBO) = reconstruction term − KL(q(z|x)||p(z)); KL prevents posterior collapse or over-regularization depending on weighting.
...
This lesson is part of the **Vaes** section in the Generative AI handbook. It treats **ELBO & KL Regularization** as an engineering topic: definitions, system shape, code you can adapt, and production constraints (quality, latency, cost, safety).
...
**ELBO & KL Regularization** — The Evidence Lower Bound (ELBO) = reconstruction term − KL(q(z|x)||p(z)); KL prevents posterior collapse or over-regularization depending on weighting.

---

## Why It Matters

ELBO terms explain blurry reconstructions and posterior collapse.
...
|----------|------------------|
| Product feature | Ship elbo & kl regularization as a user-facing capability |
...
1. Free bits / β annealing.
2. Posterior collapse when KL→0.
3. Monitor recon vs KL curves.

---

## How It Works

At a high level, elbo & kl regularization sits in the GenAI loop: condition → sample → filter → measure.
...
**Scenario:** You need elbo & kl regularization in a production feature with a clear success bar.
