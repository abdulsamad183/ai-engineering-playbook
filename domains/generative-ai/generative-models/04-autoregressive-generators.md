---
title: "Autoregressive Generators"
description: "Autoregressive generators predict the next unit (token, pixel, codec frame) conditioned on previous units."
...
> Autoregressive generators predict the next unit (token, pixel, codec frame) conditioned on previous units.
...
This lesson is part of the **Generative Models** section in the Generative AI handbook. It treats **Autoregressive Generators** as an engineering topic: definitions, system shape, code you can adapt, and production constraints (quality, latency, cost, safety).
...
**Autoregressive Generators** — Autoregressive generators predict the next unit (token, pixel, codec frame) conditioned on previous units.

---

## Why It Matters

LLMs and many audio codecs are AR — decoding strategy is half the system.
...
|----------|------------------|
| Product feature | Ship autoregressive generators as a user-facing capability |
...
1. Left-to-right factorization.
2. KV cache for efficient decode.
3. Exposure bias vs teacher forcing.

---

## How It Works

At a high level, autoregressive generators sits in the GenAI loop: condition → sample → filter → measure.
...
**Scenario:** You need autoregressive generators in a production feature with a clear success bar.
...
def greedy_decode(logits_fn, bos_id: int, eos_id: int, max_len: int = 64) -> list[int]:
    ids = [bos_id]
    for _ in range(max_len):
        logits = logits_fn(ids)  # shape [vocab]
        nxt = max(range(len(logits)), key=lambda i: logits[i])
        ids.append(nxt)
        if nxt == eos_id:
            break
    return ids
