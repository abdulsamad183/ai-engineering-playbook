...
description: "Implicit models (classic GANs) define a sampler without an explicit density p(x)."
...
> Implicit models (classic GANs) define a sampler without an explicit density p(x).
...
This lesson is part of the **Generative Models** section in the Generative AI handbook. It treats **Implicit Generative Models** as an engineering topic: definitions, system shape, code you can adapt, and production constraints (quality, latency, cost, safety).
...
**Implicit Generative Models** — Implicit models (classic GANs) define a sampler without an explicit density p(x).

---

## Why It Matters

They can produce sharp samples but are harder to train and evaluate with likelihood.
...
|----------|------------------|
| Product feature | Ship implicit generative models as a user-facing capability |
...
1. Generator maps noise→data.
2. Discriminator provides learning signal.
3. No exact p(x) for likelihood eval.

---

## How It Works

At a high level, implicit generative models sits in the GenAI loop: condition → sample → filter → measure.
...
**Scenario:** You need implicit generative models in a production feature with a clear success bar.
