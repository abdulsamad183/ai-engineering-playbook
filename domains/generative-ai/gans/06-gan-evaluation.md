...
description: "GAN evaluation uses FID, KID, IS, precision/recall, and human studies because likelihood is unavailable."
...
> GAN evaluation uses FID, KID, IS, precision/recall, and human studies because likelihood is unavailable.
...
This lesson is part of the **Gans** section in the Generative AI handbook. It treats **GAN Evaluation** as an engineering topic: definitions, system shape, code you can adapt, and production constraints (quality, latency, cost, safety).
...
**GAN Evaluation** — GAN evaluation uses FID, KID, IS, precision/recall, and human studies because likelihood is unavailable.

---

## Why It Matters

Without metrics, training 'improvements' are anecdotes.
...
|----------|------------------|
| Product feature | Ship gan evaluation as a user-facing capability |
...
1. FID vs real set.
2. Precision/recall for fidelity vs diversity.
3. Always keep a frozen eval set.

---

## How It Works

At a high level, gan evaluation sits in the GenAI loop: condition → sample → filter → measure.
...
**Scenario:** You need gan evaluation in a production feature with a clear success bar.
