---
title: "Mode Collapse"
description: "Mode collapse is when G covers only a few modes of the data distribution, producing limited diversity."
...
> Mode collapse is when G covers only a few modes of the data distribution, producing limited diversity.
...
This lesson is part of the **Gans** section in the Generative AI handbook. It treats **Mode Collapse** as an engineering topic: definitions, system shape, code you can adapt, and production constraints (quality, latency, cost, safety).
...
**Mode Collapse** — Mode collapse is when G covers only a few modes of the data distribution, producing limited diversity.

---

## Why It Matters

Collapsed generators look good in cherry-picks but fail product coverage.
...
|----------|------------------|
| Product feature | Ship mode collapse as a user-facing capability |
...
1. Detect via recall/coverage metrics.
2. Mini-batch discrimination, unrolled GANs, better objectives.
3. Diverse seeds still collapse if modes missing.

---

## How It Works

At a high level, mode collapse sits in the GenAI loop: condition → sample → filter → measure.
...
**Scenario:** You need mode collapse in a production feature with a clear success bar.
