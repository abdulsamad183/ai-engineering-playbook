...
description: "A modality is a data type and representation (tokens, pixels, latents, waveforms, spectrograms, video frames) that a generator consumes or produces."
...
> A modality is a data type and representation (tokens, pixels, latents, waveforms, spectrograms, video frames) that a generator consumes or produces.
...
This lesson is part of the **Generative Ai Fundamentals** section in the Generative AI handbook. It treats **Modalities Overview** as an engineering topic: definitions, system shape, code you can adapt, and production constraints (quality, latency, cost, safety).
...
**Modalities Overview** — A modality is a data type and representation (tokens, pixels, latents, waveforms, spectrograms, video frames) that a generator consumes or produces.

---

## Why It Matters

Modality choice drives architecture, eval metrics, safety policy, and serving cost.
...
|----------|------------------|
| Product feature | Ship modalities overview as a user-facing capability |
...
1. Text is discrete tokens; images/audio often continuous then quantized or latent.
2. Cross-modal models map one modality into another (text→image, image→text).
3. Product specs should name input/output modalities and max sizes.

---

## How It Works

At a high level, modalities overview sits in the GenAI loop: condition → sample → filter → measure.

```mermaid
flowchart LR
  Text --> LLM
  Text --> T2I[Text-to-Image]
  Image --> VLM
  Audio --> TTS
```
...
**Scenario:** You need modalities overview in a production feature with a clear success bar.
