...
description: "Discriminative models estimate p(y|x) for labels or decisions; generative models estimate p(x) or p(x|c) so they can create new x."
...
> Discriminative models estimate p(y|x) for labels or decisions; generative models estimate p(x) or p(x|c) so they can create new x.
...
This lesson is part of the **Generative Ai Fundamentals** section in the Generative AI handbook. It treats **Discriminative vs Generative** as an engineering topic: definitions, system shape, code you can adapt, and production constraints (quality, latency, cost, safety).
...
**Discriminative vs Generative** — Discriminative models estimate p(y|x) for labels or decisions; generative models estimate p(x) or p(x|c) so they can create new x.

---

## Why It Matters

Choosing the wrong family wastes compute — classifiers do not replace generators, and generators are poor standalone decision engines.
...
|----------|------------------|
| Product feature | Ship discriminative vs generative as a user-facing capability |
...
1. Use discriminative models for routing, moderation, and ranking.
2. Use generative models for synthesis and open-ended completion.
3. Hybrid stacks are normal: generate then discriminate (filter/rerank).

---

## How It Works

At a high level, discriminative vs generative sits in the GenAI loop: condition → sample → filter → measure.

```mermaid
flowchart TB
  X[Input x] --> D[Discriminative p(y|x)]
  Noise --> G[Generative p(x|c)]
  C[Condition c] --> G
```
...
**Scenario:** You need discriminative vs generative in a production feature with a clear success bar.
