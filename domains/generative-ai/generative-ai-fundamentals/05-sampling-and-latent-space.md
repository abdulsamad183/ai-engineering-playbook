---
title: "Sampling & Latent Space"
description: "Sampling draws outputs from the model; latent spaces are compressed continuous codes where many modern generators operate."
...
> Sampling draws outputs from the model; latent spaces are compressed continuous codes where many modern generators operate.
...
This lesson is part of the **Generative Ai Fundamentals** section in the Generative AI handbook. It treats **Sampling & Latent Space** as an engineering topic: definitions, system shape, code you can adapt, and production constraints (quality, latency, cost, safety).
...
**Sampling & Latent Space** — Sampling draws outputs from the model; latent spaces are compressed continuous codes where many modern generators operate.

---

## Why It Matters

Sampling hyperparameters (steps, temperature, guidance, seed) dominate user-visible quality and cost.
...
|----------|------------------|
| Product feature | Ship sampling & latent space as a user-facing capability |
...
1. Seeds enable reproducibility for debugging and A/B.
2. Latent diffusion samples in VAE space then decodes to pixels.
3. Temperature / top-p for text; CFG / steps for diffusion.

---

## How It Works

At a high level, sampling & latent space sits in the GenAI loop: condition → sample → filter → measure.

```mermaid
flowchart LR
  Z[Latent z] --> Decode --> X[Sample x]
  Params[steps / temp / CFG] --> Decode
```
...
**Scenario:** You need sampling & latent space in a production feature with a clear success bar.
