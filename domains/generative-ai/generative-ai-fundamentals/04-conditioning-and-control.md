---
title: "Conditioning & Control"
description: "Conditioning injects side information (prompt, class, image, mask, audio) so generation targets a constrained subset of the data manifold."
...
> Conditioning injects side information (prompt, class, image, mask, audio) so generation targets a constrained subset of the data manifold.
...
This lesson is part of the **Generative Ai Fundamentals** section in the Generative AI handbook. It treats **Conditioning & Control** as an engineering topic: definitions, system shape, code you can adapt, and production constraints (quality, latency, cost, safety).
...
**Conditioning & Control** — Conditioning injects side information (prompt, class, image, mask, audio) so generation targets a constrained subset of the data manifold.

---

## Why It Matters

Unconditional samples are rarely products; control is the product feature.
...
|----------|------------------|
| Product feature | Ship conditioning & control as a user-facing capability |
...
1. Classifier-free guidance (CFG) trades diversity for prompt adherence.
2. ControlNets / adapters add spatial or structural constraints.
3. Over-conditioning can collapse diversity or amplify prompt artifacts.

---

## How It Works

At a high level, conditioning & control sits in the GenAI loop: condition → sample → filter → measure.

```mermaid
flowchart TB
  Cond[Prompt / image / mask] --> Encoder
  Noise --> Generator
  Encoder --> Generator --> Sample
```
...
**Scenario:** You need conditioning & control in a production feature with a clear success bar.
