---
title: "DDPM & DDIM"
description: "DDPM is the classic stochastic sampler; DDIM is a deterministic/non-Markovian sampler that enables fewer steps and latent inversion."
...
> DDPM is the classic stochastic sampler; DDIM is a deterministic/non-Markovian sampler that enables fewer steps and latent inversion.
...
This lesson is part of the **Diffusion Models** section in the Generative AI handbook. It treats **DDPM & DDIM** as an engineering topic: definitions, system shape, code you can adapt, and production constraints (quality, latency, cost, safety).
...
**DDPM & DDIM** — DDPM is the classic stochastic sampler; DDIM is a deterministic/non-Markovian sampler that enables fewer steps and latent inversion.

---

## Why It Matters

DDIM-style samplers are how products cut latency.
...
|----------|------------------|
| Product feature | Ship ddpm & ddim as a user-facing capability |
...
1. Fewer steps with DDIM.
2. Inversion for editing.
3. Stochasticity vs determinism.

---

## How It Works

At a high level, ddpm & ddim sits in the GenAI loop: condition → sample → filter → measure.
...
**Scenario:** You need ddpm & ddim in a production feature with a clear success bar.
