...
description: "Latent diffusion runs the diffusion process in a compressed VAE latent space, decoding to pixels only at the end."
...
> Latent diffusion runs the diffusion process in a compressed VAE latent space, decoding to pixels only at the end.
...
This lesson is part of the **Diffusion Models** section in the Generative AI handbook. It treats **Latent Diffusion** as an engineering topic: definitions, system shape, code you can adapt, and production constraints (quality, latency, cost, safety).
...
**Latent Diffusion** — Latent diffusion runs the diffusion process in a compressed VAE latent space, decoding to pixels only at the end.

---

## Why It Matters

This is the Stable Diffusion pattern that made high-res t2i practical.
...
|----------|------------------|
| Product feature | Ship latent diffusion as a user-facing capability |
...
1. VAE encode → diffuse → decode.
2. UNet/DiT in latent space.
3. Text conditioning via cross-attn.

---

## How It Works

At a high level, latent diffusion sits in the GenAI loop: condition → sample → filter → measure.

```mermaid
flowchart LR
  Image --> VAE_E --> Latent --> UNet --> Latent2 --> VAE_D --> ImageOut
  Text --> Cond --> UNet
```
...
**Scenario:** You need latent diffusion in a production feature with a clear success bar.
