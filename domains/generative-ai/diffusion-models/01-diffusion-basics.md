...
description: "Diffusion models learn to reverse a gradual noising process, generating samples by iterative denoising from pure noise."
...
> Diffusion models learn to reverse a gradual noising process, generating samples by iterative denoising from pure noise.
...
This lesson is part of the **Diffusion Models** section in the Generative AI handbook. It treats **Diffusion Basics** as an engineering topic: definitions, system shape, code you can adapt, and production constraints (quality, latency, cost, safety).
...
**Diffusion Basics** — Diffusion models learn to reverse a gradual noising process, generating samples by iterative denoising from pure noise.

---

## Why It Matters

Diffusion (especially latent diffusion) is the default high-quality image/audio/video generator family.
...
|----------|------------------|
| Product feature | Ship diffusion basics as a user-facing capability |
...
2. Noise prediction networks.
3. Many steps ≈ quality/cost knob.

---

## How It Works

At a high level, diffusion basics sits in the GenAI loop: condition → sample → filter → measure.

```mermaid
flowchart LR
  X0[Clean x0] -->|noise| XT[Noise xT]
  XT -->|denoise| X0p[Sample]
```
...
**Scenario:** You need diffusion basics in a production feature with a clear success bar.
...
def linear_beta_schedule(T: int = 1000, beta_start=1e-4, beta_end=0.02) -> torch.Tensor:
    return torch.linspace(beta_start, beta_end, T)

def q_sample(x0: torch.Tensor, t: torch.Tensor, betas: torch.Tensor, noise: torch.Tensor | None = None):
    if noise is None:
        noise = torch.randn_like(x0)
    alphas = 1.0 - betas
    alpha_bar = torch.cumprod(alphas, dim=0)
    a = alpha_bar[t].sqrt().view(-1, 1)
    b = (1 - alpha_bar[t]).sqrt().view(-1, 1)
