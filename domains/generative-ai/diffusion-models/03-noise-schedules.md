...
description: "Noise schedules define β_t / SNR over time and strongly affect training speed and sample quality."
...
> Noise schedules define β_t / SNR over time and strongly affect training speed and sample quality.
...
This lesson is part of the **Diffusion Models** section in the Generative AI handbook. It treats **Noise Schedules** as an engineering topic: definitions, system shape, code you can adapt, and production constraints (quality, latency, cost, safety).
...
**Noise Schedules** — Noise schedules define β_t / SNR over time and strongly affect training speed and sample quality.

---

## Why It Matters

A bad schedule looks like a bad model.
...
|----------|------------------|
| Product feature | Ship noise schedules as a user-facing capability |
...
1. Linear, cosine, sigmoid schedules.
2. Zero terminal SNR tricks.
3. Match schedule to resolution/modality.

---

## How It Works

At a high level, noise schedules sits in the GenAI loop: condition → sample → filter → measure.
...
**Scenario:** You need noise schedules in a production feature with a clear success bar.
