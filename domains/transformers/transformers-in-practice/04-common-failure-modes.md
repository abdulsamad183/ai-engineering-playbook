...
description: "Hallucinations, prompt brittleness, length limits, and eval blind spots."
domain: transformers
tags: [transformers-in-practice, transformers]
status: published
created: 2026-08-11
updated: 2026-08-12
version: "2.0"
...
> Hallucinations, prompt brittleness, length limits, and eval blind spots.
...
This lesson is part of the **Transformers In Practice** track in the **Transformers** handbook. Treat **Common Failure Modes** as something you can implement, measure, and ship — not only define.

**Typical workflow:** HF/API → fine-tune or prompt → evaluate → deploy.
...
**Common Failure Modes** — Hallucinations, prompt brittleness, length limits, and eval blind spots.
...
Without a crisp grip on common failure modes, teams either overcomplicate baselines or under-diagnose failures. This topic shows up in interviews, design reviews, and production incidents.
...
**Scenario:** You must apply **Common Failure Modes** on a real dataset with a deadline.
...
```python
def pick_checkpoint(task: str) -> str:
    return {
        "classify": "bert-base-uncased",
        "generate": "gpt2",
        "seq2seq": "t5-small",
    }.get(task, "bert-base-uncased")
...
**Q: How do you know common failure modes is the right tool?**
