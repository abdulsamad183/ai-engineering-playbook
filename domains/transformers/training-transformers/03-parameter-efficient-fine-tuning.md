...
description: "LoRA/adapters — train tiny modules instead of all weights."
domain: transformers
tags: [training-transformers, transformers]
status: published
created: 2026-08-11
updated: 2026-08-12
version: "2.0"
...
> LoRA/adapters — train tiny modules instead of all weights.
...
This lesson is part of the **Training Transformers** track in the **Transformers** handbook. Treat **Parameter-Efficient Fine-Tuning** as something you can implement, measure, and ship — not only define.

**Typical workflow:** pretrain objective → tokens → large-batch optim → eval.
...
**Parameter-Efficient Fine-Tuning** — LoRA/adapters — train tiny modules instead of all weights.
...
Without a crisp grip on parameter-efficient fine-tuning, teams either overcomplicate baselines or under-diagnose failures. This topic shows up in interviews, design reviews, and production incidents.
...
```mermaid
flowchart LR
  Corpus --> Tokenizer --> Pretrain --> Adapt
```
...
**Scenario:** You must apply **Parameter-Efficient Fine-Tuning** on a real dataset with a deadline.
...
```python
def mlm_labels(input_ids, mask_token_id, p=0.15):
    import torch
    labels = input_ids.clone()
    prob = torch.full(labels.shape, p)
...
**Q: How do you know parameter-efficient fine-tuning is the right tool?**
